			...
			if (response.status === 500) {
				if (response.data.errorMessage) {				
					...
					if ($rootScope.modals < 1) {
						$rootScope.modals++;						
						$injector.get('$uibModal').
						open({template: '<div style="text-align:center;margin-bottom:0px;" class="alert alert-warning"><strong>Warning!</strong><br>'+ response.data.filter +'.</div>'});
					...
					$injector.get('$uibModal').
					open({template: '<div style="text-align:center;margin-bottom:0px;" class="alert alert-danger"><strong>Error!</strong><br>'+ response.statusText +'.</div>'});
				}
			}
